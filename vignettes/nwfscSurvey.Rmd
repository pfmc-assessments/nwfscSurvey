...
PlotBio.fn(dir = getwd(), 
           ...
           ...)
```
The Biomass.fn returns a list with the second element containing the design-based index of abundance. The design based index is calculated based on the defined stratas. The function writes a csv file inside the dir input location to a "forSS" folder. The PlotBio.fn plots the calculated design based index from the Biomass.fn. If `dir` does not equal `NULL`, then a "plot" folder will be created in the directory location and a png of the plot will be saved.
