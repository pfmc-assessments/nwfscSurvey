...
#'
#' @param file      a CSV file containing length comp information
...
#' @export
#' @seealso \code{\link{readInLengthComps.fn}}, \code{\link{SS3LF.fn}}

GetLFs.fn <- function(file, headerRow, lgthBins=1, gender=3, nSamps="EnterNsamps",
                      fleet="EnterFleet", season=1, partition=0, NAs2zero=TRUE,
                      sep=",", sexRatioUnsexed=NA, maxSizeUnsexed=NA) {
  len <- readInLengthComps.fn(file, headerRow=headerRow, sep=sep)
  SS3LF.fn(len, lgthBins=lgthBins, gender=gender,
           nSamps=nSamps, fleet=fleet, season=season,
           partition=partition, NAs2zero=NAs2zero, sexRatioUnsexed=sexRatioUnsexed,
