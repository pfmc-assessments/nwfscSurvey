...
#' @seealso This function uses \code{\link{create_strata}()}
#' to generate the strata returned by this function.
#'
#' @author Chantel Wetzel and Kelli Faye Johnson
#'
get_strata_area_default <- function(
  area = "coast"
) {
  ...
  ca <- create_strata(
    ...
    depths_shallow = shalm,
    depths_deep = deepm,
    lats_south = rep(CA[1], 2),
    lats_north = rep(CA[2], 2)
  )
  or <- create_strata(
    names = c("shallow_or", "deep_or"),
    depths_shallow = shalm,
    depths_deep = deepm,
    lats_south = rep(OR[1], 2),
    lats_north = rep(OR[2], 2)
  )
  wa <- create_strata(
    names = c("shallow_wa", "deep_wa"),
    depths_shallow = shalm,
    depths_deep = deepm,
    lats_south = rep(WA[1], 2),
    lats_north = rep(WA[2], 2)
  )
  if (area == "coast") {
    area <- "wa_or_ca"
  }
  ...
  sablefish <- create_strata(
    ...
    depths_shallow = rep(shalm, 3),
    depths_deep = rep(c(shalm[2], deepestm), 3),
    lats_south = rep(c(CA[1], 36, CA[1]), each = 2),
    lats_north = rep(c(WA[2], WA[2], 36), each = 2)
  )

  north_south <- create_strata(
    names = c(outer(
      c("shallow", "deep"),
      c("coast", "north", "south"),
      paste,
    ...
    depths_shallow = rep(shalm, 3),
    depths_deep = rep(c(shalm[2], deepm[2]), 3),
    lats_south = rep(c(CA[1], 40.166667, CA[1]), each = 2),
    lats_north = rep(c(WA[2], WA[2], 40.166667), each = 2)
  )

  # other standard strata
  medium <- create_strata(
  ...
  deep <- create_strata(
    ...
    depths_shallow = c(shalm, deepm[2]),
    depths_deep = c(deepm, deepestm),
    lats_south = rep(CA[1], 3),
    lats_north = rep(WA[2], 3)
  )
  north <- create_strata(
    names = c("shallow_north", "deep_north"),
    depths_shallow = shalm,
    depths_deep = deepm,
    lats_south = c(40.166667, 40.166667),
    lats_north = c(49.0, 49.0)
  )
  south <- create_strata(
    names = c("shallow_south", "deep_south"),
    depths_shallow = shalm,
    depths_deep = deepm,
    lats_south = c(32.0, 32.0),
    lats_north = c(40.166667, 40.166667)
  )
