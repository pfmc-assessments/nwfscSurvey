...
PlotBioStrata.fn <-function(dir = NULL, dat, CI=0.95, scalar=1e6, gap=0.03, ylab="Biomass ('000 mt)", xlab="Year", 
    ...
    if (dopng) {
      ...
      if ( is.null(survey.name)) { png(paste0(dir, "/plots/designed_based_by_strata_index.png"), height=7, width=7, units="in",res=300) }
      if (!is.null(survey.name)) { png(paste0(dir, "/plots/", survey.name, "_designed_based_by_strata_index.png"), height=7, width=7, units="in",res=300) } 
    }
    if(is.null(strata.names)){strata.names = names(bio_strat)}
    
    if(!is.null(mfrow.in)) { 
      if(length(mfrow.in) == 1) { par(mfrow = c(mfrow.in[1], mfrow.in[1])) }
      if(length(mfrow.in) != 1) { par(mfrow = c(mfrow.in[1], mfrow.in[2])) } }
    if(is.null(mfrow.in)) { par(mfrow = c(length(bio_strat) / 2, 2)) }
