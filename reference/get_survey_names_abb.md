...
``` r
get_survey_names_abb(surveys = "Combo", na.return = "")
```
...
for using the first two columns of
[`get_survey_names_long()`](get_survey_names_long.md), so be weary of
using terms used for multiple surveys as only the first match will be
returned for each input value.

## See also

See [`get_survey_names_long()`](get_survey_names_long.md) for a list of
available surveys.
