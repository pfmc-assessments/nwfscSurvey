...
observations within each strata to calculate a design based index. The
new [`get_design_based()`](get_design_based.md) function is more robust
and will return zeros in each strata-year combination with no
observations. However, it can be useful to review how many tows and
positive tows are present by year and strata in your data to ensure that
the selected strata for expanding the data is reasonable (e.g., avoiding
